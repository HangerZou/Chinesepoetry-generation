    ...
    for idx, poem in enumerate(poems):
        ...
        if len(sentences) == 4:
            ...
            for sentence in sentences:
                ...
                segs = [seg for seg in segmenter.segment(
                    sentence) if seg in ranks]
                if 0 == len(segs):
                    flag = False
                    break
                keyword = reduce(
                    lambda x, y: x if ranks[x] < ranks[y] else y, segs)
    ...
    with codecs.open(train_path, 'r', 'utf-8') as fin:
        ...
        while not stop:
            ...
            if batch_size != len(batch_s[0]):
                print('Batch incomplete with size {}, expecting size {}, dropping batch.'.format(
                    len(batch_s[0]), batch_size))
                break
            else:
        ...
        while not stop:
            ...
            for i in range(batch_size):
                ...
                if not line:
                    ...
                    keywords_ints = process_sentence(
                        keywords, rev=rev, pad_len=4 if align else None)
