...
class Seq2SeqModel:
    ...
    def __init__(self, config, mode):
        ...
        self.keep_prob_placeholder = tf.placeholder(
            self.dtype, shape=[], name='keep_pro')
    ...
    # 创建单个cell(不能直接放进MultiRNN列表中，模型会发生错误)
    ...
    def build_encoder(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            if self.bidirectional:
                ...
                encoder_last_state_zipped = list(
                    zip(self.encoder_last_state_fw, self.encoder_last_state_bw))
    ...
    def build_train_decoder(self):
        ...
        if self.train_mode == 'ground_truth':
            ...
            raise NotImplementedError(
                'Train mode: {} is not yet implemented'.format(self.train_mode))
    ...
    def build_predict_decoder(self):
        ...
        if not self.use_beamsearch_decode:
            ...
            if self.predict_mode == 'sample':
                ...
                raise NotImplementedError(
                    'Predict mode: {} is not yet implemented'.format(self.predict_mode))
        ...
        # 调用dynamic_decode进行解码，decoder_outputs_decode是一个namedtuple，
        ...
        if not self.use_beamsearch_decode:
            ...
            raise NotImplementedError(
                '{} mode is not recognized.'.format(self.mode))
    ...
    def train(self, sess, encoder_inputs, encoder_inputs_length,
        ...
        # Understand keep prob
        input_feed[self.keep_prob_placeholder.name] = self.keep_prob

        output_feed = [
            self.train_op,   # Update Op that does optimization
